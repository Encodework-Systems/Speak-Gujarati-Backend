...
class UserController {
  ...
  register = async (req, res, next) => {
    ...
    if (!errors.isEmpty()) {
      ...
      try {
        ...
        if (isExistUser) {
          if (isExistUser.isDeleted === 1) {
            var msg =
              "Your account was deleted by you. Please contact us if you would like to restore it.";
            throw new httpException(403, msg, {
              errors: { title: "Account is inactive", msg: msg },
            });
          }
  ...
  forgetPassword = async (req, res, next) => {
    ...
    try {
      const user = await db.user.findOne({ email: email.trim() });
  ...
  logout = async (req, res, next) => {
    ...
    if (!errors.isEmpty()) {
      ...
      try {
        //  await mSession.startTransaction();
        /// BATCH PROCESS START
        console.log("Device:", req.currentUser);
        await db.user.device.findByIdAndUpdate(req.currentUser._id, {
          isLogout: true,
          notificationToken: "",
        });

        await db.user.findByIdAndUpdate(req.currentUser._id, {
          oneSignalToken: null,
        });
  ...
  googleLogin = async (req, res, next) => {
    ...
    try {
      ...
      if (isExistUser) {
        ...
        const password = md5(
          req.body.providerId + process.env.SECRET_JWT || "",
        );
        let isUserAvailable = new db.user({
          fullname: req.body.fullname,
          provider: "google",
        ...
        await isUserAvailable.save();
        isExistUser = await db.user.findById(isUserAvailable._id);
  ...
  getAllTopics = async (req, res, next) => {
    try {
      ...
      var response = commonUtils.successRes(
        200,
        "Topics and lessons fetched successfully.",
      );
      response["data"] = topicsWithLessons;
      res.status(response["statuscode"]).send(response);
    } catch (error) {
      if (error.statuscode !== undefined) {
        throw new httpException(error.statuscode, error.msg, error.data);
  ...
  getPhrases = async (req, res, next) => {
  try {
    const { lessonId } = req.body; 

    if (!lessonId) {
      throw new httpException(400, "LessonId is required", {
        errors: "lessonId missing",
      });
    }

    // check lesson exists (optional but recommended)
    const lesson = await db.lessons.findById(lessonId);
    if (!lesson) {
      throw new httpException(404, "Lesson not found", {
        errors: "Invalid lessonId",
      });
    }
    ...
    if (!phrases || phrases.length === 0) {
      throw new httpException(404, "No phrases found", {
        errors: "Phrases not available for this lesson",
      });
    }

    var response = commonUtils.successRes(
      200,
      "Phrases fetched successfully."
    );
  ...
  facebookLogin = async (req, res, next) => {
    ...
    try {
      ...
      const { id: providerId, email, name: fullname } = fbResponse.data;
  ...
  loginWithApple = async (req, res, next) => {
    ...
    if (!errors.isEmpty()) {
      ...
      try {
        ...
        // Decode JWT payload
        const payloadBuffer = Buffer.from(payloadEncoded, "base64");
        const jwtPayload = JSON.parse(payloadBuffer.toString("utf8"));
        ...
        if (jwtPayload.iss !== "https://appleid.apple.com") {
          ...
          errors.push({
            location: "body",
            path: "access_token",
            msg: ["Token was not issued by Apple"],
          });
          errors = commonUtils.mergeErrors(errors);
          throw new httpException(422, "Validation failed", {
            errors: errors,
        ...
        if (jwtPayload.aud !== "com.telepathy.mindgames") {
          var errors = [];
          errors.push({
            location: "body",
            path: "token",
            msg: ["Token was not issued for the correct client ID"],
          });
          errors = commonUtils.mergeErrors(errors);
          throw new httpException(422, "Validation failed", {
            errors: errors,
        ...
        if (jwtPayload.error && jwtPayload.error !== "") {
          var errors = [];
          errors.push({
            location: "body",
            path: "token",
            msg: ["Invalid oauth2 from Apple"],
          });
          errors = commonUtils.mergeErrors(errors);
          throw new httpException(422, "Validation failed", {
            errors: errors,
        ...
        req.body.firstname = req.body.firstname
          ? req.body.firstname
          : "Telepathy";
        req.body.lastname = req.body.lastname ? req.body.lastname : "Master";
        req.body.providerId = jwtPayload.sub;
        ...
        if (!isExistUser) {
          isExistUser = await db.user.findOne({
            loginId: req.body.providerId,
            type: 1,
          });
        }
        //.session(mSession);
        ...
        if (isExistUser) {
          ...
          var userDevice = await this.saveUserDeviceInfo(isExistUser._id, req);

          var extraDevices = await db.user.device
            .find({
              _id: {
                $ne: userDevice._id,
              },
          ...
          for (let index = 0; index < extraDevices.length; index++) {
            await db.user.device.findByIdAndUpdate(extraDevices[index]._id, {
              isLogout: true,
              isForceLogout: true,
            ...
            await commonUtils.logout(isExistUser._id, extraDevices[index]._id);
          }
          ...
          var password = md5("Enc0dew0rk");
          var username = await commonUtils.generateCoolUsername();
          ...
          var country = await db.country.findOne({
            code: countryCode,
          });
  ...
  saveSubscriptionInfo = async (req, res, next) => {
    var errors = validationResult(req);
